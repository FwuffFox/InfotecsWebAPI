    ...
    public class ValueDtoValidator : AbstractValidator<ValueDto>
    {
        public ValueDtoValidator()
        {
            ...
            RuleFor(x => x.ExecutionTime)
                ...
                .Must(executionTime =>
                {
                    if (decimal.TryParse(executionTime, out var parsedExecutionTime)) return parsedExecutionTime >= 0;
                    return false;
                })
            ...
            RuleFor(x => x.Value)
                ...
                .Must(value =>
                {
                    if (decimal.TryParse(value, out var parsedValue)) return parsedValue >= 0;
                    return false;
                })
